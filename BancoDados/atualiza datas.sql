select f1.co_fluxo_caixa,
       f1.dt_vencimento,
       DATE_ADD((select dt_vencimento from TB_FLUXO_CAIXA f2 where f1.co_fluxo_caixa = f2.co_fluxo_caixa), INTERVAL +12
                month) as novo_vencimento,
       f1.dt_cadastro,
       DATE_ADD((select dt_cadastro from TB_FLUXO_CAIXA f2 where f1.co_fluxo_caixa = f2.co_fluxo_caixa), INTERVAL +12
                month) as novo_cadastro,
       f1.dt_realizado,
       DATE_ADD((select dt_realizado from TB_FLUXO_CAIXA f2 where f1.co_fluxo_caixa = f2.co_fluxo_caixa), INTERVAL +12
