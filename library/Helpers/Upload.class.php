...
class Upload
{
    ...
    //Verifica e monta o nome dos arquivos tratando a string!
    private function setFileName2()
    {
        $FileName = Valida::ValNome($this->Name) . '-' . uniqid() . strrchr($this->File['name'][0], '.');
        $this->Name = $FileName;
    }
    ...
    {
        if (move_uploaded_file($this->File['tmp_name'][0], self::$BaseDir . $this->Folder . $this->Name)):
            $this->Result = $this->Name;
            $this->Error = null;
        else:
            $this->Result = false;
     ...
     * <b>Enviar Mídia:</b> Basta envelopar um $_FILES de uma mídia e caso queira um nome e um tamanho personalizado.
     * Caso não informe o tamanho será 40mb!
     * @param FILES $Media = Enviar envelope de $_FILES (XML)
     * @param STRING $Name = Nome do arquivo ( ou do artigo )
     * @param STRING $Folder = Pasta personalizada
     * @param STRING $MaxFileSize = Tamanho máximo do arquivo (40mb)
     */
    public function UploadArquivoXML(array $midia, $Name = null, $Folder = null, $MaxFileSize = null)
    {
        $this->File = $midia;
        $this->Name = ((string)$Name ? $Name : substr($midia['name'][0], 0, strrpos($midia['name'][0], '.')));
        $this->Folder = ((string)$Folder ? $Folder : 'medias');
        $MaxFileSize = ((int)$MaxFileSize ? $MaxFileSize : 40);
        ...
        if ($this->File['size'][0] > ($MaxFileSize * (1024 * 1024))):
            $this->Result = false;
            $this->Error = "Arquivo muito grande, tamanho máximo permitido de {$MaxFileSize}mb";
        elseif (!in_array($this->File['type'][0], $FileAccept)):
            $this->Result = false;
            $this->Error = 'Tipo de arquivo não suportado. Envie .XML!';
        else:
            $this->CreateFolder($this->Folder);
            $this->setFileName2();
            $this->MoveFile2();
            $this->Result = 'Arquivo Transferido com Sucesso!';
            $this->Error = false;
        endif;
