...
//         GRAFICO MAPA
    ...
    public function DetalharVisitas()
    {
        ...
        $coPagina = UrlAmigavel::PegaParametro(CO_PAGINA);

        if ($coPagina) {
            /** @var PaginaEntidade $pagina */
            $pagina = $PaginaService->PesquisaUmRegistro($coPagina);
            /** @var PaginaVisitaEntidade $pagVisita */
            foreach ($pagina->getCoPaginaVisita() as $pagVisita) {
                /** @var TrafegoEntidade $trafego */
                $trafego = $TrafegoService->PesquisaUmRegistro($pagVisita->getCoVisita()->getCoTrafego());
                $pagVisita->getCoVisita()->setCoTrafego($trafego);
            }
            $this->result = $pagina->getCoPaginaVisita();
        } else {
            Notificacoes::geraMensagem(
                'Detalhes das Visitas dessa Página não encontrado!',
                TiposMensagemEnum::ALERTA
            );
            Redireciona(UrlAmigavel::$modulo . '/' . UrlAmigavel::$controller . '/ListarVisita/');
        }
    }
}
